...
from .models import Turf, Facility, Sports, SportField, TimeSlot, Price, SlotEligibility,Review,Favorite
...
class ReviewSerializer(serializers.ModelSerializer):
    ...
    user_id = serializers.PrimaryKeyRelatedField(queryset=UserModel.objects.all(), write_only=True)
    class Meta:
        model = Review
        fields = ['id', 'user', 'turf','user_id', 'rating', 'comment', 'created_at']
...
class FavoriteSerializer(serializers.ModelSerializer):
    user = serializers.PrimaryKeyRelatedField(queryset=UserModel.objects.all())
    turf = serializers.PrimaryKeyRelatedField(queryset=Turf.objects.all())  # Show ID when creating/updating
    marked_at = serializers.DateTimeField(read_only=True)

    class Meta:
        model = Favorite
        fields = ['id', 'user', 'turf', 'marked_at']

    def to_representation(self, instance):
        # Override the default to_representation method to customize the output on GET requests
        representation = super().to_representation(instance)
        
        # If we're serializing a 'turf' field, use the TurfSerializer to include detailed info
        turf_serializer = TurfSerializer(instance.turf)
        representation['turf'] = turf_serializer.data
