...
class TurfBookingSerializer(serializers.ModelSerializer): 
    ...
    def validate(self, attrs):
        coupon_code = attrs.get('coupon', None)
        is_paid_full = attrs.get('is_paid_full', False)
        advance_payable = attrs.get('advance_payable', 500)
        if advance_payable < 500:
            raise serializers.ValidationError("The advance payable must be at least 500.")
            ...
            try:
                ...
                if coupon_code:
                    try:
                        ...
                        attrs['total_amount'] = max(0, total_amount - discount)
...
class BadmintonBookingSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if advance_payable < 200:
            raise serializers.ValidationError("The advance payable must be at least 200.")
        if badminton_slot:
            try:
