...
class ApplyCoupon(models.Model):
    user = models.ForeignKey(UserModel, on_delete=models.CASCADE)
    turf_slot = models.ForeignKey(TurfSlot, on_delete=models.CASCADE, related_name='applied_coupons',null=True)
    Badminton_Slot = models.ForeignKey(BadmintonSlot, on_delete=models.CASCADE, related_name='applied_coupons',null=True)
    Swimming_Slot = models.ForeignKey(SwimmingSlot, on_delete=models.CASCADE, related_name='applied_coupons',null=True)
    ...
    def __str__(self):
        ...
        if self.turf_slot:
            ...
            slot = self.Badminton_Slot
        elif self.Swimming_Slot:
            slot = self.Swimming_Slot

        return f"{self.user} applied {self.coupon.code} on {slot if slot else 'No Slot'}"
