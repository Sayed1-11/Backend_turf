...
class ApplyCoupon(models.Model):
    user = models.ForeignKey(UserModel, on_delete=models.CASCADE)
    turf_slot = models.ForeignKey(TurfSlot, on_delete=models.CASCADE, related_name='applied_coupons')
    coupon = models.ForeignKey(Coupon, on_delete=models.CASCADE)
    discount_applied = models.DecimalField(max_digits=6, decimal_places=2, default=0.00)
    applied_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"{self.user} applied {self.coupon.code} on {self.turf_slot}"