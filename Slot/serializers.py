...
class TurfSlotSerializer(serializers.ModelSerializer):
    ...
    def get_calculated_price(self, obj):
        try:
            return obj.calculate_price()  # Call the calculate_price method from the TurfSlot model
...
class SwimmingSlotSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SwimmingSlot
        fields = ['id', 'user', 'turf', 'field', 'date', 'is_booked', 'is_available', 'coupon', 'session', 'number_of_people','calculated_price']
        read_only_fields = ['is_booked', 'is_available', 'calculated_price']
    def get_calculated_price(self, obj):
        try:
            return obj.calculate_price()  # Call the calculate_price method from the TurfSlot model
