...
class BaseSlot(models.Model):
    ...
    def get_price(self):
        ...
        if not slot_ids:
            raise ValidationError( f"No overlapping time slot found for the selected time range: {self.start_time} to {self.end_time}.")
        total_price = 0
        ...
        if slot_ids:
            ...
            if price_entry:
                ...
                raise ValidationError(f"Price not defined for time slots {slot_ids}, duration {duration} hours, and day {day_of_week} and turf - {self.turf} in {self.field}.")
        else:
