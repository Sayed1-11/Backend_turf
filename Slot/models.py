...
class BaseSlot(models.Model):
    ...
    def get_price(self):
        ...
        # Check if the first slot ID exists before querying
        if slot_ids[0]:
            print("Slot ID is valid: ", slot_ids[0])
            
            # Fetch price entries
            price_entries = Price.objects.filter(
            turf=self.turf,  # Corrected from 'Turf' to 'turf'
            field=self.field,
...
class SwimmingSlot(models.Model):
    ...
    def calculate_price(self):
        # For SwimmingSlot, you might want to use session-based pricing instead
...
def create_hourly_sessions():
    # Define the start and end of the day (e.g., 6:00 AM to 9:00 PM)
    ...
    while current_time < end_of_day:
        ...
        session = SwimmingSession.objects.create(
            ...
            end_time=end_time,
            capacity=20,  # 20 persons per session
            price_per_person=200.00  # Set your desired price here
        )
        session.save()
